...
const buildListItem = (item) => { //we create dom elements
...
const addClickListenerToCheckbox = (checkbox) => {
    checkbox.addEventListener("click", (event) => {
        ...
        updatePersistentData(toDoList.getList());
        const removedText = getLabeledText(checkbox.id);
...
const getLabeledText = (checkboxId => {
...
const calcNextItemId = () => {
    ...
    const list = toDoList.getList();
    if (list.lenght > 0) {
        nextItemId = list[list.length - 1].getId() + 1; //Hay una falla en el conteo del ID
    }
    return nextItemId;
};
